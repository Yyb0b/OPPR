...
class OASVR:
    ...
    def learn(self, newSampleX, newSampleY):
        ...
        while not newSampleAdded:
            ...
            # print("flag:", flag, "minIndex:", minIndex)
            if len(self.supportSetIndices) > 0 and len(beta)>0:
                # print("deltaC:", deltaC)
    ...
    def findMinVariation_delete(self, H, beta, gamma, i):
        """ Finds the variations of each sample to the new set.
        Lc1: distance of the new sample to the SupportSet
        Lc2: distance of the new sample to the ErrorSet
        Ls(i): distance of the support samples to the ErrorSet/RemainingSet
        ...
        minLeIndex = np.abs(Le).argmin()
        minLrIndex = np.abs(Lr).argmin()
        minIndices = [None, minLsIndex, minLeIndex, minLrIndex]
        minValues = np.array([Lc2, Ls[minLsIndex], Le[minLeIndex], Lr[minLrIndex]])
        print("minValues:", minValues)
        if np.abs(minValues).min() == np.inf:
            print('No weights to modify! Something is wrong.')
            sys.exit()
    ...
    def adjustSets_delete(self, H, beta, gamma, i, flag, minIndex):
        print('Entered adjustSet logic with flag {0} and minIndex {1}.'.format(flag,minIndex))
        if flag not in range(4):
            print('Received unexpected flag {0}, exiting.'.format(flag))
            sys.exit()
        # add new sample to Support set
        if flag == 0:
            print('Moving delete sample {0} to remainder set.'.format(i))
            if self.weights[i] != 0:
                print("self.weights[i]:不为0=",self.weights[i])
            self.weights[i] = 0
            return H, True
        # move sample from Support set to Error or Remainder set
        elif flag == 1: 
    ...
    def delete(self, i):
        if i in self.remainderSetIndices:
            self.remainderSetIndices.remove(i)
            self.X.pop(i)
            self.Y.pop(i)
            self.weights = np.delete(self.weights, i)
            for j in range(len(self.supportSetIndices)):
                if self.supportSetIndices[j] > i:
                    self.supportSetIndices[j] -= 1
            for j in range(len(self.errorSetIndices)):
                    if self.errorSetIndices[j] > i:
                        self.errorSetIndices[j] -= 1
            self.numSamplesTrained -= 1
        else:
            H = self.computeMargin(self.X, self.Y)
            SampleDeleted = False
            iterations = 0
            while not SampleDeleted:
                # Ensure we're not looping infinitely
                iterations += 1
                ...
                beta, gamma = self.computeBetaGamma(i)
                # Find minimum variation and determine how we should shift samples between sets
                deltaC, flag, minIndex = self.findMinVariation_delete(H, beta, gamma, i)
                print("deltaC:", deltaC)
                print("flag: ", flag)
                print("minIndex", minIndex)
                H, SampleDeleted = self.adjustSets_delete(H, beta, gamma, i, flag, minIndex)
                # Update weights and bias based on variation
                if len(self.supportSetIndices) > 0 and len(beta)>0:
                    if i in self.supportSetIndices:
                        self.weights[i] += deltaC
                        delta = beta*deltaC
                        self.bias += delta.item(0)
                    # numpy is wonky...
                        weightDelta = np.array(delta[1:])
                        weightDelta.shape = (len(weightDelta),)
                        tmp = self.supportSetIndices
                        # tmp.remove(i)
                        self.weights[tmp] += weightDelta
                        H += gamma*deltaC
                    else:
                        self.weights[i] += deltaC
                        delta = beta*deltaC
                ...
                # Adjust sets, moving samples between them according to flag
                print(self.weights)
            print(self.remainderSetIndices)
            # self.remainderSetIndices.remove(i)
            self.X.pop(i)
            self.Y.pop(i)
            self.weights = np.delete(self.weights, i)
            ...
            for j in range(len(self.errorSetIndices)):
                    if self.errorSetIndices[j] > i:
                        self.errorSetIndices[j] -= 1
            for j in range(len(self.remainderSetIndices)):
                    if self.remainderSetIndices[j] > i:
                        self.remainderSetIndices[j] -= 1
            self.numSamplesTrained -= 1
